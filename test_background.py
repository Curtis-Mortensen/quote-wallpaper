...
def get_quote(db_path='quotes.db'):
    ...
    # Connect and fetch a random quote
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("SELECT id, quote FROM quotes ORDER BY RANDOM() LIMIT 1")
    row = cur.fetchone()
    conn.close()
    return (row[0], row[1]) if row else (0, "No quote found.")
...
def choose_font_for_text(text, draw, max_width, max_height, font_path, max_font_size=300, min_font_size=10, max_lines=7):
    ...
    current_max = min(max_font_size, int(max_height / 7))  # Ensure even the largest font could fit 7 lines
...
def main():
    ...
    # Define text area inside the plaque with a smaller inner margin
    text_margin = int(min(plaque_width, plaque_height) * 0.1)  # 10% of smaller dimension
    text_area_x = plaque_x + text_margin + plaque_inset
    text_area_y = plaque_y + text_margin + plaque_inset
    text_area_width = plaque_width - 2 * (text_margin + plaque_inset)
    text_area_height = plaque_height - 2 * (text_margin + plaque_inset)

    # Get a quote from the database
    quote_id, quote_text = get_quote('quotes.db')
